...
def plot_drone_routes(drones, charging_stations, blocks, routes, fcr, ugv_factor, ugv_init_loc = None, save_ = False):
    """
    ...
    if blocks is not None:
        for block in blocks:
            polygon = Polygon(block, facecolor='black', alpha=0.8, hatch='////', edgecolor='black', linewidth=1, label='Obs.')
    ...
    def draw_route_segment(start_point, end_point, color, style, blocks, range_):
        ...
        if blocks is not None:
            for block in blocks:
                ...
                extra_points = obstacle.find_additional_length([start_point, end_point], Path_=True)
                if extra_points:
        ...
        if additional_points:
            ...
            for i in range(len(full_path) - 1):
                ax.plot([full_path[i][0], full_path[i+1][0]], 
        ...
        if dist > range_:
            ...
            ax.text(mid_x, mid_y, f"{dist / range_:.2f}", color=text_color, ha="left", va="center")
        else:
            text_color = 'green'
    ...
    for drone_idx, (drone, route, color) in enumerate(zip(drones, routes, colors)):
        ...
        # Add annotations
        ax.text( init_x, init_y, f'V{drone_idx+1} ({charge})',
                   ha='center', va='bottom')
        ax.text(dest_x, dest_y, f'D{drone_idx+1}', 
                   ha='center', va='bottom',
                    zorder=2)
    
    # Plot charging stations last (highest z-order)
    for i, station in enumerate(charging_stations):
        ax.plot(station[0], station[1], '^', color='blue', markersize=10, zorder=3, label='UGV')
        ax.annotate(f'F{i+1}', (station[0], station[1]), 
                   xytext=(5, 5), textcoords='offset points',
                    zorder=3)
    if not ugv_init_loc is None:
        plt.scatter(ugv_init_loc[0,0],ugv_init_loc[0,1],color='orange',marker='2')
        plt.text(ugv_init_loc[0,0], ugv_init_loc[0,1], 'Initial UGV location', ha='center', va='bottom')
    ...
    if save_:
          plt.savefig(f'new sim res for M={len(charging_stations)} F.C.R. = {fcr} alpha = {ugv_factor}.pdf')
    plt.show()