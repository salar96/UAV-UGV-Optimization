...
def calc_associations(D_ss, beta):
    ...
    for D_s in D_ss:
        ...
        for i in range(1, K + 1):
            out_D[i] = D[i - 1] + np.repeat(
                np.transpose(out_D[i - 1]), D[i - 1].shape[0], axis=0
            )
            m = out_D[i].min(axis=1, keepdims=True)
            exp_D = np.exp(np.multiply(-beta, out_D[i] - m))
            out[i] = np.sum(
                np.multiply(exp_D, np.tile(out[i - 1], (1, D[i - 1].shape[0])).T),
                axis=1,
                keepdims=True,
            )
...
def free_energy(D_s, P_s, beta):
    ...
    for i in range(1, K + 1):
        ...
        out_P[i] = (
            (
                P[i - 1]
                * np.repeat(np.transpose(out_P[i - 1]), P[i - 1].shape[0], axis=0)
            )
            .sum(axis=1)
            .reshape(-1, 1)
        )
        out_C[i] = (
            (
                ...
                * (
                    D[i - 1]
                    * np.repeat(np.transpose(out_P[i - 1]), D[i - 1].shape[0], axis=0)
                    + np.repeat(np.transpose(out_C[i - 1]), D[i - 1].shape[0], axis=0)
                )
            )
            .sum(axis=1)
            .reshape(-1, 1)
        ...
        out_H[i] = (
            -(
                ...
                * (
                    my_log(P[i - 1])
                    * np.repeat(np.transpose(out_P[i - 1]), D[i - 1].shape[0], axis=0)
                    - np.repeat(np.transpose(out_H[i - 1]), D[i - 1].shape[0], axis=0)
                )
            )
            .sum(axis=1)
            .reshape(-1, 1)
