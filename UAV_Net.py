...
class UAV_Net:
    ...
    def return_stagewise_cost(self, params, beta):  # params is like stations
        ...
        for drone_id, drone in enumerate(self.drones):
            stage = np.concatenate(
                (params, np.array(self.drones[drone_id][1]).reshape(1, -1)), axis=0
            )
            ...
            d_f_to_delta = cdist(
                params, np.array(self.drones[drone_id][1]).reshape(1, -1), self.distance
            )
            # strt = time.time()
            if not self.blocks == None:
                for block in self.blocks:
                    obs = Obstacle(block)
                    d_f_to_delta = d_f_to_delta + cdist(
                        params,
                        np.array(self.drones[drone_id][1]).reshape(1, -1),
                        metric=add_block_dist(obs),
                    )
            # print('3',time.time()-strt)
            d_last = np.concatenate(
                (d_f_to_delta, np.array([0]).reshape(1, -1)), axis=0
            )
    ...
    def objective(self, params, beta):
        ...
        if self.ugv_factor == 0.0:
            ...
            return cost + self.ugv_factor * np.linalg.norm(
                params.reshape(-1, 2) - self.stations
            )
    ...
    def return_direct_cost(self):
        return np.sum(
            [
                np.sum((np.array(drone[0]) - np.array(drone[1])) ** 2) ** 0.5
                for drone in self.den_drones
            ]
        )
